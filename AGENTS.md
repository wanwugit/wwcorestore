...
- **Refactor status (verified 2026-07-31):** the balance `ChangeAsync` idempotent/transactional core is implemented and tested (`CoreCmsUserBalanceServicesTests`, 11 passing, SQLite in-memory). The commission state machine (docs 03/04) is **paper-only** — `CommissionStatus` enum does not exist, `CoreCmsDistributionOrder.status` field is not added, L2–L4 tests are not implemented. Production decision: ship in **downgrade mode** — distribution commission settlement and refund-clawback flows are gated off by `Distribution:CommissionSettleEnabled` in `appsettings.json` (default `"0"` = disabled). The gate is enforced as an early-return at the top of `CoreCmsDistributionOrderServices.AddData` / `FinishOrder` / `CancleOrderByOrderId`; the parallel agent-commission path (`CoreCmsAgentOrderServices`) is intentionally NOT gated. Set the key to `"1"` only after the state machine lands and L2–L4 tests pass.
